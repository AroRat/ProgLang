...
Без оптимизации программа создаёт все переменные в памяти и честно прокручивает цикл 123 раза, постоянно 
проворачивая операцию вычисления.  
С первым уровнем оптимизации переменные переместились в ячейки процессора (edx, eax) и постоянное
вычисление было заменено на однократное умножение. Только вот цикл не был удалён и программа просто так
прогоняла цикл 123 раза.   
На втором уровне оптимизации цикл всё же удаляется и вычисление упаковывается в одну инструкцию.